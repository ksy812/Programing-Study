...
FROM emp
WHERE deptno=30;
--hiredate+3는 현재날짜에 3을 더한 결과를 날짜로 출력
--hiredate+5/24 날짜에 정수를 더하면 해당 일수가 되는데, 24로 나눌 경우 시간이 더해진다.
--1일이 24시간이기 때문에 24로 나누면 시간이 된다.
...
--예제3 - EXTRACT함수
SELECT
    EXTRACT(day from sysdate) 일자, --현재 날짜에서 일을 출력
    EXTRACT(month from sysdate) 월, --현재 날짜에서 월을 출력
    EXTRACT(year from sysdate) 년도 --현재 날짜에서 년도를 출력
FROM dual;

--예제4
SELECT 
    SYSTIMESTAMP A,
    EXTRACT(HOUR FROM SYSTIMESTAMP) B, --현재 시간에서 9를 뺀 값을 반환. 당연함.
    TO_CHAR(SYSTIMESTAMP, 'HH24') C --HH24는 24시간제 시간을 출력
FROM dual;
--TIME ZONE: 경도 0도에 있는 그리니치 천문대를 기준으로한 시각의 차이
--          대한민국은 9를 더함,
...
SELECT 
    ...
    EXTRACT(TIMEZONE_HOUR FROM SYSTIMESTAMP) BTZH, --대한민국의 타임존은 9이다.
    EXTRACT(HOUR FROM SYSTIMESTAMP) EH, --현재시간에서 9를 뺀 결과 출력
    EXTRACT(HOUR FROM CAST(SYSTIMESTAMP AS TIMESTAMP)) EHC
    --SYSTIMESTAMP를 TIMESTAMP로 형변환해서 시간을 뽑아내면 현재시간이 출력된다.
FROM dual;

--예제6
SELECT  SYSTIMESTAMP,
        EXTRACT(hour FROM SYSTIMESTAMP) h1, --현재 날짜에서 일수를 출력
        EXTRACT(hour FROM SYSTIMESTAMP) +  --현재 날짜에서 월을 출력
        EXTRACT(TIMEZONE_HOUR FROM SYSTIMESTAMP) h2 --현재 날짜에서 시간을 출력
FROM dual;
...
SELECT  ename, hiredate, sysdate,
        ...
        --현재날짜에서 입사일자 사이의 개월수의 소수점 이하 버림
FROM emp
WHERE deptno = 10
...
FROM emp
WHERE deptno in(10, 30) --부서번호가 10 또는 30번인 사원
ORDER BY hiredate desc;

--예제3
SELECT  ename, hiredate,
        next_day(hiredate, 6) n_6, --입사일자 이후의 금요일에 해당하는 날짜를 출력
...
SELECT
        next_day(SYSDATE, 6) n_6, --오늘 날짜 이후의 금요일 날짜 출력
        next_day(SYSDATE, 7) n_7 --오늘 날짜 이후의 토요일 날짜 출력
FROM DUAL;

--예제4
SELECT  empno, ename, hiredate, last_day(hiredate) l_last, --입사한 달의 마지막 날짜 출력
        last_day(hiredate) - hiredate l_day --입사한 달의 근무일수
FROM emp
ORDER BY last_day(hiredate) - hiredate desc;

--예제5
SELECT to_char(sysdate, 'YY/MM/DD HH24:MI:SS') normal,
        --오늘 날짜를 'YY/MM/DD HH24:MI:SS' 형식으로 변환
        to_char(trunc(sysdate), 'YY/MM/DD HH24:MI:SS') trunc,
        --시간 부분을 0으로 초기화
        to_char(round(sysdate), 'YY/MM/DD HH24:MI:SS') round
        --오늘 날짜의 시간 부분을 반올림(12시 이상이면 올림) 일(날)수 증가
FROM dual;

--예제6
SELECT  hiredate,
        ROUND(hiredate, 'MONTH'),
        --입사일자의 일자가 15이상이면 그 다음달 1일로 출력되고,
        --입사일자의 월이 6월 이상이면 그 다음 년도의 1월 1일 출력됨.
        ROUND(hiredate, 'YEAR') --날짜의 월 부분에서 반올림
FROM emp
WHERE deptno=20;
